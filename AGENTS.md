...
See [`FORMATTING.md`](./FORMATTING.md) for the full, detailed CSS/build-script formatting spec (page setup, fonts, spacing, caption/heading rules, footnote mechanics and gotchas, TOC/landscape-table implementation, citation conventions, etc.) and the specific bugs each convention exists to prevent. Any future change to `PETITION.md`'s structure or `build_pdf.py`'s CSS should read that file first and preserve its conventions unless the user says otherwise — several of them were only discovered after shipping a real, user-reported bug (doubled footnote numbers, unclickable links, a code-block-rendered list, a table row silently losing data), so reverting one without understanding why it's there tends to reintroduce the same bug.
